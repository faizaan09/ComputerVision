...
def Bonus_cylindrical_warping(img1, img2, img3):
    ...
    mask = np.zeros_like(img1, dtype='float32')
    mask[:, int(img1.shape[1] * 0.45):] = 1
    interim = Laplacian_Pyramid_Blending_with_mask(img1, im3_warp, mask, 3)
    ...
    im2_warp = cv2.warpAffine(img2, M, (interim.shape[1], interim.shape[0]))
    m2_warp = cv2.warpAffine(mask2, M, (interim.shape[1], interim.shape[0]))
    mask = np.zeros_like(img1, dtype='float32')
    mask[:, int(img1.shape[1] * 0.56):] = 1

    out = Laplacian_Pyramid_Blending_with_mask(im2_warp, interim, mask, 3)
